...
background_image = pygame.transform.scale(
    pygame.image.load("17.02.2026/glory.jpeg"),
...
class Sprite(pygame.sprite.Sprite):
    ...
    def __init__(self, color, height, width):
        ...
        pygame.draw.rect(self.image, color, pygame.Rect(0, 0,
                                                         width, height))
        self.rect = self.image.get_rect()

    def move(self, x_change, y_change):
        self.rect.x = max(
            min(self.rect.x + x_change, SCREEN_WIDTH -
                self.rect.width), 0)
        self.rect.y = max(
...
while running:
    ...
    # Display win message
    if won:
        win_text = font.render("You win!", True,
                               pygame.Color('Red'))
        screen.blit(win_text, ((SCREEN_WIDTH -
                                win_text.get_width()) // 2,
                               (SCREEN_HEIGHT -
                                win_text.get_height()) // 2))
